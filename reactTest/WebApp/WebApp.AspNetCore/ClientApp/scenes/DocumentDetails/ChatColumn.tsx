...
import { Navbar, Nav, NavItem, NavDropdown, MenuItem, Glyphicon, FormGroup, FormControl, Button, Modal, ControlLabel } from 'react-bootstrap';
...
class ChatColumn extends React.Component<ReduxProps, null> {
    ...
    renderChats() {
        ...
        return allMessages.map(msg => {
            ...
            return <TimelineEvent createdAt={toLocalDateStringUS(msg.time)}
                title={<span><b>{msg.caller}</b></span>}
                icon={<div className={'text-icon ' + iconColor}>{msg.caller.charAt(0)}</div>}
                contentStyle={{ backgroundColor: 'white', borderRadius: '5px' }}
                cardHeaderStyle={{ background: '#e4effb', color: 'inherit' }}
                container="card">
                {msg.message}
            </TimelineEvent>
        });

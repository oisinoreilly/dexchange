...
import { Permission } from '../../backendTypes';
...
export class RoleViewer extends React.Component<ReduxProps, state>{
    state = {
        checked: [],
        expanded: [],
        nodes: []
    ...
    componentWillReceiveProps(nextProps: ReduxProps) {
        ...
        if (!hasPermissions) {
            ...
            const permittedBanks: any[] = this.getPermitted('Bank', this.props.banks.bankList, nextProps.roles.selectedRole.Permissions);
            const permittedAccounts: any[] = this.getPermitted('Account', this.props.accounts.accountList, nextProps.roles.selectedRole.Permissions);
            const permittedCorporates: any[] = this.getPermitted('Corporate', this.props.corporates.corporateList, nextProps.roles.selectedRole.Permissions);

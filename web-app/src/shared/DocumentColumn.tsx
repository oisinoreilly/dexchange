...
import { Navbar, Nav, NavItem, NavDropdown, MenuItem, Glyphicon, FormGroup, FormControl, Button, ButtonGroup, Modal, ControlLabel } from 'react-bootstrap';
...
import StatusToColour, { StatusColour, SortByStatus } from '../typeMappings';
...
export interface IDocumentColumnProps {
    ...
    addDocument: (accountId: string, document: DocumentUi) => void;
    selectDocument: (documentIndex: string) => void;
    uploadDocument: (documentIndex: string, contentBase64: string) => void;
    deleteDocument: (documentId: string) => void;
    requestContent: (documentId: string, contentId: string) => void;
}
...
export class DocumentColumn extends React.Component<IDocumentColumnProps, IDocumentState> {
    constructor(props) {
        super(props);
        this.state = {
   ...
   handleDocOwnerChange = (e) => {
       this.setState({
           owner: e.target.value,
       });
   };

   handleDocDaysOverdueChange = (e) => {
       this.setState({
   ...
   modalDeleteClose = () => {
    ...
    handleChangeDocumentUpload = (evt) => {
    ...
    onUploadDocumentClick = (document: DocumentUi) => {
    ...
    public render() {
                ...
                <form>
                    <FormGroup controlId="formBasicText" validationState={this.getValidationState()}>
                        ...
                        <FormControl
                            ...
                            onChange={this.handleDocTitleChange}
                        />
                    ...
                    <FormGroup controlId="formBasicFile">
                        ...
                        <FormControl
                            ...
                            onChange={this.handleChangeDocumentUpload}
                        />
                        <FormControl.Feedback />
                    </FormGroup>
                </form>
        ...
        const selectedId = this.props.selectedDocument ? this.props.selectedDocument.id : "";
            ...
            return <DocumentColumnItem
                key={i}
                onClick={() => this.onDocumentClick(document.id)}
                onDocumentClick={() => this.onUploadDocumentClick(document)}
                selected={document.id == selectedId}
                document={document}
                icon={docIcon}
                onDelete={() => this.modalDeleteSelectedOpen()}
                requestContent={() => this.props.requestContent(document.id, document.documentContentId)}
            >
            </DocumentColumnItem>
        });
